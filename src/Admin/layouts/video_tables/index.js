...
function Tables() {
  ...
  const fetchData = async (page, search, Short) => {
    try {
      const response = await Api.getAllVideo(page, search, Short);
      if (response && response.Videos.data && Array.isArray(response.Videos.data)) {
        const datas = response.Videos.data;
        // Modify the data here to match the new structure
        const modifiedData = datas.map((user) => {
          setTotal_rows(response.Videos.total);
          setFirstpageurl(response.Videos.first_page_url);
          setFrom(response.Videos.from);
          setLastpage(response.Videos.last_page);
          setLastpageurl(response.Videos.last_page_url);
          setLinks(response.Videos.links);
          setNextpageurl(response.Videos.next_page_url);
          setPath(response.Videos.path);
          setPer_page(response.Videos.per_page);
          setPrevpageurl(response.Videos.prev_page_url);
          setTotal(response.Videos.total);
          setCurrentpage(response.Videos.current_page);
          return {
            _id: user._id,
  ...
  const renderSuccessSB = (
    <MDSnackbar
      ...
      content={"Video Status Update Successfully"}
      dateTime="0 Sec ago"
      open={successSB}
  ...
  const VideoAdminStatusUpdate = async (videoId) => {
    try {
      const response = await Api.VideoAdminStatus(videoId);
      if (response) {
        setSuccessSB(true);
      }
    } catch (error) {
                      ...
                      <DataTable
                        table={{
                          ...
                          rows: userData.map((user) => ({
                            ...
                            View_Count: (
                              <Link to={`/view-count-list/${user._id}/Video`}>
                                {user.view_count}
                              </Link>
                            ),
                            Comment_Count: user.comment_count,
                            ...
                            Url: (
                              <MDTypography
                                component="a"
                                href={`/website-video-comment/${user._id}`}
                              ...
                              >
                                {`${base_url}/website-video-comment/${user._id}`}
                              </MDTypography>
                            ),
                            action: (
                              <MDTypography
