...
function Tables() {
    ...
    try {
      ...
      if (response && response.payments && Array.isArray(response.payments)) {
        ...
        const modifiedData = datas.map((user) => {
          ...
          const formattedCreatedAt = new Date(user.createdAt).toLocaleString("en-IN", {
            year: "numeric",
            month: "long",
            day: "numeric",
            hour: "numeric",
  ...
  const handleUpdate = async (value, id, type) => {
    console.log(value);
    console.log(id);
    console.log(type);
    try {
      ...
      if (type === "master_single") {
        response = await Api.updateMasterSinglePayment(updatedData);
      } else if (type === "master_group") {
        response = await Api.updateMasterGroupPayment(updatedData);
      } else if (type === "advance_single") {
        response = await Api.updateAdvanceSinglePayment(updatedData);
      } else if (type === "advance_group") {
        response = await Api.updateAdvanceGroupPayment(updatedData);
      } else {
                            ...
                            Payment: (
                              <div>
                                ...
                                <Button
                                  ...
                                  size="small"
                                  variant="contained"
                                  onClick={() => handleUpdate(user.Payment, user._id, user.Type)}
                                >
                                  Update
                                </Button>
                              </div>
